-- Demo data for SQLite: the same rows and the same fixed ids as data-mariadb.sql,
-- which tests rely on. Loaded where spring.sql.init.mode is always - the dev and
-- test profiles - and idempotent (spec 9.3). Change both files together.
--
-- Written in the forms the sqlite profile makes the driver write (see
-- V6__baseline.sql): instants and dates as 'yyyy-MM-dd HH:mm:ss.SSS' text in UTC,
-- money as text. A date written as '2026-11-01' alone would not compare equal to
-- the '2026-11-01 00:00:00.000' the application binds, and would not parse back.
--
...
INSERT INTO jobs (id, created_at, last_modified_at, employer_id, title, description, url,
  ...
  ('8f14e45f-ceea-467a-9b4f-3a1b2c3d4e5f', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   ...
   '2026-11-01 00:00:00.000', NULL, '2099-10-15 00:00:00.000', 'ACTIVE', '2026-09-01 00:00:00.000'),
  ('9a25f56a-dffb-478b-ac50-4b2c3d4e5f60', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   'Praktikum Produktdesign', NULL, 'https://www.acme.example/jobs/ACME-2026-021',
   'de', 'ACME-2026-021', NULL, 'INTERNSHIP', 'HYBRID', 'JUNIOR',
   NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'ACTIVE', '2026-09-18 00:00:00.000'),
  ('ab36067b-e00c-489c-bd61-5c3d4e5f6071', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   ...
   NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '2020-01-31 00:00:00.000', 'ACTIVE', '2026-08-01 00:00:00.000'),
  ('bc47178c-f11d-490d-ce72-6d4e5f607182', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
  ...
  ('cd58289d-022e-4a1e-df83-7e5f60718201', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   ...
   NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'INACTIVE', '2025-11-04 00:00:00.000'),
  ...
  ('de69390e-133f-4b2f-e094-8f6071829301', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   ...
   NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'ACTIVE', '2026-09-19 00:00:00.000')
ON CONFLICT DO NOTHING;
...
INSERT INTO feeds (id, created_at, last_modified_at, employer_id, name, slug, description) VALUES
  ('cd58289d-022e-4a1e-df83-7e5f60718293', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   'All jobs', 'all', 'Every job this employer currently advertises.'),
  ('de69390e-133f-4b2f-e094-8f6071829304', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
...
INSERT INTO users (id, created_at, last_modified_at, issuer, subject, email, display_name, role) VALUES
  ('11111111-1111-4111-8111-111111111111', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), 'dev', 'dev@localhost',
   'dev@localhost', 'dev@localhost', 'ADMIN'),
  -- A registered colleague with no memberships, so the invite flow has a real target.
  ('22222222-2222-4222-8222-222222222222', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), 'dev', 'editor@example.com',
   'editor@example.com', 'Edith Editor', 'USER'),
  -- An actual member, so the People screen has a member to list and remove.
  ('44444444-4444-4444-8444-444444444444', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), 'dev', 'member@example.com',
...
INSERT INTO memberships (id, created_at, last_modified_at, user_id, employer_id, role) VALUES
  ('55555555-5555-4555-8555-555555555555', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'),
   '11111111-1111-4111-8111-111111111111', '003d6aec-021b-11f1-aefa-f649a5d91690', 'OWNER'),
  ('66666666-6666-4666-8666-666666666666', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'),
...
INSERT INTO invitations (id, created_at, last_modified_at, employer_id, invitee_id,
  ...
  ('33333333-3333-4333-8333-333333333333', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
...
INSERT INTO service_tokens (id, created_at, last_modified_at, employer_id, name, prefix,
  ...
  ('77777777-7777-4777-8777-777777777777', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), '003d6aec-021b-11f1-aefa-f649a5d91690',
   'ats-sync', 'ojp_seedonly01',
   '{bcrypt}$2a$10$//umO/HMS.9LMIbFuRqerehvXBa1t.SdsgCqpEgTpDAcUmNlxDS22',
   NULL, strftime('%Y-%m-%d %H:%M:%f', 'now', '+12 months'), NULL,
...
-- rather than strftime('%Y-%m-%d %H:%M:%f', 'now'): it is honest, and it keeps findByEmployerIdOrderByCreatedAtDesc
-- deterministic, which a test relies on.
INSERT INTO service_tokens (id, created_at, last_modified_at, employer_id, name, prefix,
                            secret_hash, last_used_at, expires_at, revoked_at, created_by_id) VALUES
  ('79797979-7979-4979-8979-797979797979',
   strftime('%Y-%m-%d %H:%M:%f', 'now', '-13 months'), strftime('%Y-%m-%d %H:%M:%f', 'now', '-13 months'),
   '003d6aec-021b-11f1-aefa-f649a5d91690',
   'legacy-export', 'ojp_seedexpired',
   '{bcrypt}$2a$10$//umO/HMS.9LMIbFuRqerehvXBa1t.SdsgCqpEgTpDAcUmNlxDS22',
   strftime('%Y-%m-%d %H:%M:%f', 'now', '-12 months'), strftime('%Y-%m-%d %H:%M:%f', 'now', '-1 months'), NULL,
   '11111111-1111-4111-8111-111111111111')
ON CONFLICT DO NOTHING;
...
INSERT INTO memberships (id, created_at, last_modified_at, user_id, service_token_id,
  ...
  ('88888888-8888-4888-8888-888888888888', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'), NULL,
   '77777777-7777-4777-8777-777777777777', '003d6aec-021b-11f1-aefa-f649a5d91690', 'EDITOR'),
  ('89898989-8989-4989-8989-898989898989',
   strftime('%Y-%m-%d %H:%M:%f', 'now', '-13 months'), strftime('%Y-%m-%d %H:%M:%f', 'now', '-13 months'), NULL,
